...
class FinalPDFTextInserter:
    ...
    def insert_text_at_coordinates(self, text_mappings, output_path):
        """
        Вставляет текст в PDF по заданным координатам с поддержкой кириллицы
    ...
    def _get_cyrillic_font(self):
        """Возвращает доступный шрифт с поддержкой кириллицы"""
        cyrillic_fonts = [
            'STSong-Light',
            'HeiseiMin-W3', 
            'HeiseiKakuGo-W5',
    ...
    def _insert_single_text(self, canvas_obj, text_mapping, page_number, page_width, page_height, font_name):
        """Вставляет один текст на канвас с поддержкой кириллицы"""
        ...
        if target_annotation:
            ...
            print(f'Вставлен текст: "{text}" в позицию ({center_x:.1f}, {center_y:.1f}) на странице {page_number} (шрифт: {font_name})')
        else:
...
if __name__ == "__main__":
    ...
    inserter.insert_text_at_coordinates(
        ...
        output_path='./out/final_filled_shablon.pdf'
    )
